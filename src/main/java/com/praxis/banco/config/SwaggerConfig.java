...
public class SwaggerConfig implements Serializable {
	...
	public Docket productApi() {
		return new Docket(DocumentationType.SWAGGER_2).useDefaultResponseMessages(Boolean.FALSE).select()
				.apis(RequestHandlerSelectors.basePackage("com.praxis.banco.controler"))//ubicacion del controller
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("Clientes Service").description("CRUD Cliente").version("1.0")
				.contact(new Contact("Esmeralda Cruz Garcia", " ", "ESCG@praxisglobe.com")).build();
	}
